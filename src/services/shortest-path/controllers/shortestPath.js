const geoserverUrl = 'http://localhost:8080/geoserver/wfs?authkey=' + geoServer.authkey + '&'

/* function to obtain vertice near to selected point */
function getVertice (pontoSelecionado) {
  const url = `${geoserverUrl}service=WFS&version=1.0.0&request=GetFeature&typeName=ArqSistemas:nearest_vertex&outputformat=application/json&viewparams=x:${
...
function getRota (origin, destination) {
  const url = `${geoserverUrl}service=WFS&version=1.0.0&request=GetFeature&typeName=arqsistemas:shortest_path&outputformat=application/json&viewparams=source:${origem};target:${destino};`
